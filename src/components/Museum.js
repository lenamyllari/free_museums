...
const Museum = ({museum}) => {
    ...
    const details = <div>
        ...
        {museum.services.length > 0 && <ul className="museumList">
            <b>Services: </b>{museum.services.map((service, idx) => <li
            key={idx}>{service}</li>)}</ul>}
        {museum.themes.length > 0 && <ul className="museumList">
            <b>Themes: </b>{museum.themes.map((theme, idx) => <li key={idx}>{theme}</li>)}</ul>}
    </div>
